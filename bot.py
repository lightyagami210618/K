...
ADMIN_IDS = [7954343626] # 👈 မိမိ၏ ID အစစ် ပြောင်းထည့်ရန်
...
def add_client_to_3xui(server_info, client_id_or_pass, email, gb_limit):
    ...
    if server_info['protocol'] in ['vless', 'ais', 'dtac', 'starlink']:
...
def get_main_menu():
    ...
    # New User များအတွက် အသုံးပြုနည်း လမ်းညွှန်ဗီဒီယို ခလုတ်အသစ်
    btn_how_to_use = types.InlineKeyboardButton("📖 How to use", url="https://t.me/konosuba_210618/40")
    
    # UI Layout ကို သပ်သပ်ရပ်ရပ် နေရာချခြင်း
    markup.add(btn_info, btn_reg, btn_ref, btn_gen, btn_mykeys, btn_status)
    markup.add(btn_buy) # Buy Credit ကို တစ်ကြောင်းတည်းအပြည့်ထားခြင်း
    markup.add(btn_channel, btn_how_to_use) # လင့်ခ်နှစ်ခုကို ဘေးချင်းယှဉ်လျက် အောက်ဆုံးမှာ ပြသခြင်း
    return markup
...
def handle_callbacks(call):
    ...
    if call.data == "main_menu":
        ...
        # Starlink ခလုတ်အသစ် ထပ်ထည့်ခြင်း
        markup.add(types.InlineKeyboardButton("Starlink", callback_data="proto_starlink"))
        markup.add(types.InlineKeyboardButton("🔙 Back to Main Menu", callback_data="main_menu"))
        ...
        elif server_info['protocol'] == 'starlink':
            generated_uuid = str(uuid.uuid4())
            api_success = add_client_to_3xui(server_info, generated_uuid, email_id, selected_gb)
            final_key = f"vless://{generated_uuid}@{server_info['ip']}:80?type=ws&encryption=none&flow=&host=portal-as.ruijienetworks.com&path=%2F#{selected_gb}GB_{server_info['name']}"            
        else: # Hysteria2
...
def remove_server_cmd(message):
    ...
    try:
        ...
        conn = sqlite3.connect("vpn_bot.db")
        conn.row_factory = sqlite3.Row  # Dict ပုံစံမျိုး Column အမည်နဲ့ ဖတ်ရလွယ်အောင် သတ်မှတ်ခြင်း
        cursor = conn.cursor()
        
        # ဖျက်မည့်ဆာဗာ ရှိမရှိ အရင်စစ်ဆေးခြင်း
        ...
        if server:
            ...
            # ၁။ ဒီဆာဗာကနေ ကီးဝယ်ယူထားသည့် User များ၏ Key စာရင်းကို အရင်ဆွဲထုတ်ခြင်း
            cursor.execute("SELECT telegram_id, email, total_gb FROM user_keys WHERE server_id = ?", (server_id,))
            active_keys = cursor.fetchall()
            
            # ၂။ ဆာဗာ၏ 3X-UI Panel သို့ လော့ဂင်ဝင်ပြီး Traffic စစ်ရန် ပြင်ဆင်ခြင်း
            session = login_3xui(server['panel_url'], server['username'], server['password'])
            ...
            for key in active_keys:
                ...
                # Panel အဆင်မပြေခဲ့လျှင် ဝယ်ထားသော ပမာဏအတိုင်း အပြည့်ပြန်အမ်းရန် Fallback သတ်မှတ်ခြင်း
                remaining_gb = total_gb 
                
                if session:
                    try:
                        # API မှတစ်ဆင့် User ၏ အသုံးပြုပြီးသား Data ကို လှမ်းစစ်ခြင်း
                        traf_url = f"{server['panel_url'].rstrip('/')}/panel/api/inbounds/getClientTraffics/{email}"
                        res = session.get(traf_url, timeout=4, verify=False)
                        if res.status_code == 200 and res.json().get("success"):
                            obj = res.json().get("obj", {})
                            up = obj.get("up", 0)
                            down = obj.get("down", 0)
                            used_bytes = up + down
                            used_gb = used_bytes / 1073741824
                            remaining_gb = max(0.0, total_gb - used_gb) # အနုတ်လက္ခဏာမပြအောင် ထိန်းခြင်း
                    except:
                        pass # API ပိတ်နေပါက ဝယ်ယူထားသည့် GB အတိုင်း အပြည့် ပြန်အမ်းမည်
                
                # ဒဿမ ၂ နေရာအထိ ဖြတ်ပြီး ခရက်ဒစ်အဖြစ် ပြောင်းလဲခြင်း
                refund_credits = round(remaining_gb, 2)
                
                if refund_credits > 0:
                    # ၃။ User ဆီသို့ Credits ပြန်လည်ပေါင်းထည့်ပေးခြင်း
                    cursor.execute("UPDATE users SET credits = credits + ? WHERE telegram_id = ?", (refund_credits, tg_id))
                    
                    # ၄။ သက်ဆိုင်ရာ User ဆီသို့ Auto Refund သတင်းစကားနှင့် တောင်းပန်လွှာ ပေးပို့ခြင်း
                    try:
                        apology_msg = (
                            f"⚠️ *ဝန်ဆောင်မှု ရပ်ဆိုင်းခြင်းနှင့် တောင်းပန်လွှာ*\n\n"
                            f"လူကြီးမင်းဝယ်ယူအားပေးထားသော ဆာဗာ *[{server_name}]* ကို နည်းပညာဆိုင်ရာ အဆင့်မြှင့်တင်မှုများကြောင့် "
                            f"ကျွန်ုပ်တို့၏ Bot မှ ဖယ်ရှားလိုက်ပြီ ဖြစ်ပါသဖြင့် သင်၏ နောက်ဆုံးလက်ကျန် Data "
                            f"**{refund_credits} GB** ကို **{refund_credits} Credits** အဖြစ် သင့်အကောင့်ထဲသို့ "
                            f"စနစ်မှ အလိုအလျောက် ပြန်လည်အမ်းပေး (Auto Refund) လိုက်ပါပြီဗျာ။\n\n"
                            f"အဆင်မပြေမှု ဖြစ်ပေါ်သွားသည့်အတွက် လူကြီးမင်းအား များစွာ တောင်းပန်အပ်ပါသည်ခင်ဗျာ။ 🙏"
                        )
                        bot.send_message(tg_id, apology_msg, parse_mode="Markdown")
                    except:
                        pass # User မှ Bot ကို Block ထားပါက Error မတက်ဘဲ ကျော်သွားရန်
                
                refunded_users_count += 1
            
            # ၅။ သက်ဆိုင်ရာ Key များနှင့် ဆာဗာကို Database ထဲမှ အပြီးတိုင် ဖျက်သိမ်းခြင်း
            cursor.execute("DELETE FROM user_keys WHERE server_id = ?", (server_id,))
            cursor.execute("DELETE FROM servers WHERE id = ?", (server_id,))
            conn.commit()
            
            bot.reply_to(message, f"✅ ဆာဗာ '{server_name}' (ID: {server_id}) ကို Database ထဲမှ အောင်မြင်စွာ ဖျက်ပစ်လိုက်ပါပြီ။\n\n📊 သက်ဆိုင်ရာ Customer ({refunded_users_count}) ဦးထံသို့ လက်ကျန် GB များအတိုင်း Credits များတွက်ချက်ပြီး Auto Refund နှင့် တောင်းပန်လွှာ ပေးပို့ပြီးပါပြီဗျာ။")
        else:
